...
vec4f hello_from_cpp_module( das_context * ctx, das_node * node, vec4f * args ) {
    return das_result_string("Hello from cpp module");
}
...
extern "C" {
    EXPORT_API das_module * register_dyn_Module_Hello() {
        ...
        das_module_bind_interop_function(mod, lib, &hello_from_cpp_module, "hello_from_cpp_module", "hello_from_cpp_module", SIDEEFFECTS_modifyExternal, "s ");
