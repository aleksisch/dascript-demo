...
void run() {
    ...
    das_program * program = das_program_compile("hello.das", fAccess, tout, dummyLibGroup);
    ...
    das_function * fnTest = das_context_find_function(ctx,"main");
    if ( !fnTest ) {
        das_text_output(tout, "function 'main' not found\n");
        goto shutdown;
    }
...
das_module * register_module() {
    // create module and library
    das_module * mod = das_module_create ("module");
